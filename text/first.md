% Struggling through the first, bad, list
...
Now we can focus on building our list. For now we'll avoid generics to keep things simple. We'll only support storing signed 32-bit integers:
...
we get the null pointer optimization, which *eliminates the tag*. If the variant is A, the whole enum is set to all `0`'s. Otherwise, the variant is B. This works because B can never be all `0`'s, since it contains a non-zero pointer. Slick!

So how do we avoid the extra allocation *and* get that sweet null-pointer optimization? We just have to think a little more C-like: structs (AKA product types)!
...
To add methods to a type, we use `impl` blocks.
...
`push` *mutates* the list, so we'll want to take `self` mutably. We also need to take an `i32` to push:
